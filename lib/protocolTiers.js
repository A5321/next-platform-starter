export function getProtocolTier(scope, result) {
  ...
  // All three indices are read as 0–100 scale (as returned by the API).
  //
  // HARD triggers (Exit Protocol) — chronic instability, pattern entrenched:
  //   • pattern_recurrence_probability >= 70  → cycle is likely locked in
  //   • long_term_stability_forecast   <= 35  → dynamic is structurally fragile
  //   • boundary_violation_probability >= 70  → boundaries are being crossed repeatedly
  //
  // SOFT triggers (Stabilization Protocol) — strain is visible but not yet critical:
  //   • pattern_recurrence_probability >= 45
  //   • long_term_stability_forecast   <= 55
  //   • boundary_violation_probability >= 45
  ...
  if (scope === "you-are-an-option") {
    ...
    if (
      recurrence >= 45 ||                          // recurrence теперь тоже влияет
